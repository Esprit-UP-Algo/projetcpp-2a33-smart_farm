...
QString t= QString::number(type_gc);
    ...
    query.prepare("UPDATE COMMANDES SET REF= :REF ,ENTREPRISE= :ENTREPRISE, OBJET= :OBJET, TYPE= :TYPE, CONTACT= :CONTACT, EMAIL= :EMAIL, DATE :DATE , ETAT= :ETAT WHERE REF= :REF");
    query.bindValue(":REF",res);
    query.bindValue(":ENTREPRISE",entreprise_gc);
    query.bindValue(":OBJET",objet_gc);
    query.bindValue(": TYPE",t);
