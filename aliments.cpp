...
#include <QSqlError>  // Include QSqlError after QSqlQuery
...
double Aliments::gettemperature_gal_forLait() {
    QSqlQuery query;
    query.prepare("SELECT temperature FROM ALIMENTS WHERE type_al LIKE 'lait'");

    if (query.exec() && query.next()) {
        double temperature_gal = query.value("temperature").toDouble();
        return temperature_gal;
    }
